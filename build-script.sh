...
INTERFACEVIEW=plex-blsclient_iv.aadl
grep "version => \"2" plex-blsclient_iv.aadl >/dev/null && {
    echo -e "${INFO} Converting interface view from V2 to V1.3"
    TASTE --load-interface-view plex-blsclient_iv.aadl --export-interface-view-to-1_3 __iv_1_3.aadl
...
cd "$SKELS" && rm -f dummypantiltproducer.zip && zip dummypantiltproducer dummypantiltproducer/* && cd $OLDPWD
